      ...
      <div class="row g-0 justify-content-center align-items-center">
        <div class="col-10">
          ...
          <nav class="nav nav-pills nav-justified">
            ...
            <a class="nav-link" href="tujuan_deposit.php">Tujuan Deposit</a>
            <a class="nav-link active" href="live_chat.php">Live Chat</a>
          </nav>
          <br>
          <div class="row col-md-12">

            <?php 
            $query = mysqli_query($koneksi, "SELECT * FROM live_chat");
            while ($data = mysqli_fetch_array($query)) {
              $live_chat = $data['live_chat'];
              $live_chat_wa = $data['live_chat_wa'];
              $id = $data['id'];
            }
            ?>

            <form action="function/ubah_live_chat.php" method="POST">
              <div class="form-group">
                <h4>Live Chat Online : </h4>
                <input type="hidden" name="id" value="<?php echo $id ?>">
                <input type="hidden" name="level" value="<?php echo $level ?>">
                <input type="hidden" name="nama_pengguna" value="<?php echo $nama_pengguna ?>">
                <input type="text" class="form-control" name="live_chat" required="" value="<?php echo $live_chat ?>">
              </div>
              <br>
              <div class="form-group">
                <h4>Live Chat Whatsapp : </h4>
                <input type="text" class="form-control" name="live_chat_wa" required="" value="<?php echo $live_chat_wa ?>">
              </div>
              <br>
              <div class="form-group">
                <input type="submit" name="submit" class="btn btn-warning" style="float: right;" value="Simpan Data Pengaturan Live Chat">
              </div>
            </form>
          </div>
          <br>
