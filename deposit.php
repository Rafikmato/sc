  ...
  <body>
    ...
    <div class="container-fluid px-0" style="padding: 67px 0 5rem 0;">
      ...
      <div class="row g-0 justify-content-center align-items-center">
        <div class="col-10">
          <?php 
          $query = mysqli_query($koneksi, "SELECT saldo FROM user WHERE email = '$email' AND level = 'pengguna' ");
          while ($data = mysqli_fetch_array($query)) {
            $saldo = $data['saldo'];
          }
          ?>
          <?php 
          $query3 = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE email = '$email' AND status = 'Pending' AND depo_wd = 'Deposit' ");
          $hitung = mysqli_num_rows($query3);
          if ($hitung > 0) {
            ?>
            <h3 style="color: red;">Anda Memiliki Tagihan Yang Belum Di Bayarkan</h3>
            <?php
          }
          ?>

          <?php 
          if ($level=="admin" || $level == "promotor") {
            $query = mysqli_query($koneksi, "SELECT SUM(jumlah) AS total FROM transaksi WHERE depo_wd = 'Deposit' AND status = 'berhasil' ");
            while ($data = mysqli_fetch_array($query)) {
              $saldo1 = $data['total'];
            }
            ?>
            <h1>Total Keseluruhan Deposit : Rp. <?php echo number_format($saldo1); ?></h1>
            <table>
              <tr>
                <th>No</th>
                <th>Nama Lengkap</th>
              ...
              <?php 
              $query1 = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE status = 'pending' AND depo_wd = 'Deposit' ");
              $no = 1;
              while ($data = mysqli_fetch_array($query1)) {
                ...
                <tr>
                  ...
                  <td>
                    <?php
                    $query2 = mysqli_query($koneksi, "SELECT * FROM user WHERE email = '$data_email' ");
                    while ($ss = mysqli_fetch_array($query2)) {
                      echo $ss['nama_lengkap'];
                    }
                    ?> 
                  </td>
                  ...
                  <td style="width: 250px;">
                    <a href="function/konfirmasi_transfer.php?id=<?php echo $data['id'] ?>&email=<?php echo $data['email'] ?>" class="btn btn-primary">Acc Transfer</a>
                    <a href="edit_deposit.php?id=<?php echo $data['id'] ?>&email=<?php echo $data['email'] ?>" class="btn btn-warning">Edit</a>
                    <a href="function/konfirmasi_transfer_hapus.php?id=<?php echo $data['id'] ?>&email=<?php echo $data['email'] ?>" class="btn btn-danger">Hapus</a>
                  </td>
                </tr>
                <?php
              }
            ...
            ?>
            <h1>Saldo Akun : Rp. <?php echo number_format($saldo); ?></h1>
            <div class="row col-md-12">
              <form action="function/aksi_deposit.php" method="POST">
                <div class="form-group">
                  <h4>Jumlah : </h4>
                  <input type="number" class="form-control" name="jumlah" placeholder="Minimal : Rp. 50.000" required="">
                  <input type="hidden" name="deposit" value="Deposit">
                  <input type="hidden" name="status" value="Pending">
                  <input type="hidden" name="email" value="<?php echo $email; ?>">
                  <p style="font-style: italic; color: red">*Minimal Deposit Rp. 50.000</p>
                </div>
                <div class="form-group">
                  <h4>Tujuan : </h4>
                  <select class="form-control" name="bank" required="">
                    ...
                    <?php 
                      $query4 = mysqli_query($koneksi, "SELECT * FROM bank");
                      while ($data = mysqli_fetch_array($query4)) {
                        ?>
                          <option value="<?php echo $data['nama_rekening'] ?>"><?php echo $data['nama_rekening']; ?></option>
                        <?php
                      }
                    ?>
                  </select>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="promo">
                    <option selected="" disabled="">-- Pilih Promo --</option>
                    <option value="depo_100">BONUS DEPOSIT 300K SALDO 600K TO X 1</option>
                    <option value="depo_200">BONUS DEPOSIT 500K SALDO 1,2JT TANPA TO</option>
                    <option value="depo_300">CLAIM BONUS NEW MEMBER 100%</option>
                    <option value="depo_500">BONUS CASHBACK JIKA KALAH 90% MINGGUAN</option>
                    <option value="depo_500">CLAIM BONUS NEW MEMBER 400% TANPA TO</option>
                    <option value="depo_500">TANPA BONUS</option>
                    <option value="depo_500">NEW MEMBER DEPOSIT 150.000 BONUS 500.000 TANPA TO</option>
                    <option value="depo_500">AUTO MAXWIN DEPOSIT 500.000</option>
                  </select>
                  <p style="font-style: italic; color: red">*PERSYARATAN INI HANYA BERLAKU UNTUK NEW MEMBER</p>
                </div>
                <br>
                <div class="form-group">
                  <input type="submit" name="submit" class="btn btn-warning" style="float: right;">
                </div>
              </form>
            </div>
            <br>
            ...
            <table>
              ...
              <?php 
              $query1 = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE email = '$email' AND depo_wd = 'Deposit' ORDER BY id DESC ");
              $no = 1;
              while ($data = mysqli_fetch_array($query1)) {
                ?>
                <tr>
